...
void send_back(pid_t sender_pid) {
    if (mode == SIGQUEUE) {
        ...
        for (int i=1; i<=recieved; ++i) {
            ...
            printf("CATCHER: SEND SIGUSR1 WITH SIGQUEUE\n");
        }
        value.sival_int = recieved;
        sigqueue(sender_pid, SIGUSR2, value);
        printf("CATCHER: SEND SIGUSR2 WITH SIGQUEUE\n");
    }
    else {
        ...
        for (int i=0; i<recieved; ++i) {
            ...
            printf("CATCHER: SEND SIGUSR1 OR SIGRTMIX WITH KILL\n");
        }
        kill(sender_pid, sig2);
        printf("CATCHER: SEND SIGUSR2 OR SIGRTMAX WITH KILL\n");
    }
